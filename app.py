...
local_server = True
app = Flask(__name__)
app.secret_key = "A1conding-super"
...
class Contacts(db.Model):
    # sno,name,email,phone_num,date,mes
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=box_title, slug=slug, tagline=tline,
...
def contact():
    if (request.method == 'POST'):
        ...
        entry = Contacts(name=name, phone_num=phone, mes=message,
